/**
 * Fluvio Iframe Widget
 * Optimised for embedding inside <iframe> elements.
 * Uses a fixed root wrapper instead of raw position:fixed on FAB/panel,
 * and pins the host body so the iframe page never scrolls.
 *
 * Usage:
 * <script src="https://your-domain.com/fluvio-iframe-widget.js"
...
(function() {
  ...
  const currentScript = document.currentScript || document.querySelector('script[src*="fluvio-iframe-widget"]');
  ...
  if (window.FluvioIframeWidgetLoaded) {
    console.warn('Fluvio Iframe Widget already loaded');
    return;
  }
  window.FluvioIframeWidgetLoaded = true;
  ...
  function injectStyles() {
    ...
    const css = `
      /* ── Iframe root wrapper — eliminates scrollbar-from-fixed-elements quirk ── */
      #fluvio-iframe-root {
        position: fixed;
        inset: 0;
        pointer-events: none;
  ...
  function createWidget() {
    ...
    iframeRoot.id = 'fluvio-iframe-root';
    iframeRoot.classList.add(`fluvio-position-${config.position}`);
    ...
    iframeRoot.appendChild(fab);
    iframeRoot.appendChild(panel);
